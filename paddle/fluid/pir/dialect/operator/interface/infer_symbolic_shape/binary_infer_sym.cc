...
bool DotOpInferSymbolicShape(pir::Operation *op,
  ...
  PADDLE_ENFORCE_EQ(1 == x_rank || 2 == x_rank, true, common::errors::InvalidArgument("ShapeError: The dimensions of input tensor X (%u) should be 1 or 2", x_rank));

  auto y_dims = infer_context->GetShapeOrDataForValue(op->operand_source(1)).shape();
  auto y_rank = y_dims.size();
  PADDLE_ENFORCE_EQ(x_rank == y_rank, true, common::errors::InvalidArgument("ShapeError: The rank of input tensor Y (%u) must match that of input tensor X(%u).", y_dims.size(), x_rank));
  ...
  PADDLE_ENFORCE_EQ(shape_match, true, common::errors::InvalidArgument("ShapeError: The dimension of input tensors X(%u) and Y(%u) are different", x_rank, y_rank));
